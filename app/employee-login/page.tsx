...
export default function Page() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/employee-portal/login", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({email,password}) });
      const d = await res.json();
      if (!res.ok) setError(d.error || "Login failed");
      else { router.push("/employee-portal"); router.refresh(); }
    ...
    <div style={{minHeight:"100vh",background:"var(--page-bg)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div className="anim-up" style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <h1 style={{fontSize:22,fontWeight:700,color:"var(--tx-primary)",letterSpacing:"-0.02em"}}>Employee Portal</h1>
          <p style={{fontSize:13.5,color:"var(--tx-tertiary)",marginTop:5}}>View and manage your tasks</p>
        </div>
        <div className="card" style={{padding:24,boxShadow:"var(--shadow-md)"}}>
          {error && <div style={{marginBottom:14,padding:"9px 12px",background:"var(--red-bg)",borderRadius:"var(--r-md)",color:"var(--red)",fontSize:13}}>({error})</div>}
        ...
        <div style={{display:"flex",justifyContent:"center",gap:24,marginTop:18}}>
          <a key="/login" href="/login" style={{fontSize:12.5,color:"var(--tx-tertiary)",textDecoration:"none",transition:"color .12s"}} onMouseEnter={e=>(e.currentTarget.style.color="var(--tx-primary)")} onMouseLeave={e=>(e.currentTarget.style.color="var(--tx-tertiary)")}>Admin login</a>
          <a key="/client-login" href="/client-login" style={{fontSize:12.5,color:"var(--tx-tertiary)",textDecoration:"none",transition:"color .12s"}} onMouseEnter={e=>(e.currentTarget.style.color="var(--tx-primary)")} onMouseLeave={e=>(e.currentTarget.style.color="var(--tx-tertiary)")}>Client login</a>
        </div>
      </div>
    </div>
  );
